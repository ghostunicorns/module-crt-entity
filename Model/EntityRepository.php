...
use GhostUnicorns\CrtEntity\Api\Data\EntitySearchResultInterfaceFactory;
use GhostUnicorns\CrtEntity\Api\EntityRepositoryInterface;
use GhostUnicorns\CrtEntity\Model\EntityModelFactory as EntityFactory;
use GhostUnicorns\CrtEntity\Model\ResourceModel\Entity\EntityCollection;
use GhostUnicorns\CrtEntity\Model\ResourceModel\Entity\EntityCollectionFactory;
...
class EntityRepository implements EntityRepositoryInterface
{
    ...
    public function getById(int $id): EntityInterface
    {
        ...
        if (!$entity->getId()) {
            throw new NoSuchEntityException(__('Unable to find CrtEntity with ID "%1"', $id));
        }
        return $entity;
    }
    ...
    public function getAllDataRefinedByActivityIdGroupedByIdentifier(int $activityId): array
    {
        ...
        foreach ($entities as $entity) {
            ...
            $type = $entity->getType();
            $refinedData = $this->serializer->unserialize($entity->getDataRefined());
            if (!array_key_exists($identifier, $result)) {
                $result[$identifier] = [];
            }
            $result[$identifier][$type] = $refinedData;
        }
